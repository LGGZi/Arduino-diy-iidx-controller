...
void EncL(){
  ...
  xAxis_buf = (uint8_t)((int16_t)(encL / encoder_sensitivity) % 256) - 128;
}
void EncR(){
  //简单的将编码器脉冲顺增逆减后储存在encL里
  //Serial.print("+p");
  ...
  yAxis_buf = (uint8_t)((int16_t)(encR / encoder_sensitivity) % 256) - 128;
}
//}
...
void EncL_AbsoluteOn(){
  ...
  if(startencl>numSTARTENCL){
    if (digitalRead(ENC_L_B) != HIGH) {
      ...
      buttonstate |= (uint16_t)1 << (max_bt + 1);
    } else {
      xAxis_buf = -125;
...
void EncR_AbsoluteOn(){
  ...
  if(startencR>numSTARTENCL){
    if (digitalRead(ENC_R_B) != HIGH) {
      ...
      buttonstate |= (uint16_t)1 << (max_bt + 3);
    } else {
      yAxis_buf = -125;
...
void EncR_AbsoluteOff(){
  //绝对值模式的归中函数
  if(stopFlagR < stopThreshold){
      stopFlagR++;
    } else {
      yAxis_buf = 0;
      startencR=0;
...
void using_Controller_style(){
  ...
  button_buf = button_scan();
  buttonstate &= button_buf + (0b1111 << max_bt);
  ...
  report.xboxButtons = buttonstate | button_scan();
  //模式为绝对值模式时利用此函数归中
  if(stopThreshold != 0){
    EncL_AbsoluteOff();
    EncR_AbsoluteOff();
  ...
  report.lxAxis = xAxis_buf;

  report.lyAxis = yAxis_buf;

  // 可开关的灯光部分
  lightswitch(11,0,3,button_buf);

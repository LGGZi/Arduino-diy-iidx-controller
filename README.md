...
* 模式4：键鼠模式，七个按键映射为键盘`z s x d c f v`，转盘映射为鼠标x轴和y轴，非常适合用于本家<br>

------------------------------------------------------------------------------------<br>
通过使用EEPROM实现了模式的记忆，插入时不选择模式（不按下按键）会从EEPROM中读取上次的模式并进入
可利用组合键开关灯光，开关灯光的函数为`lightswitch()`，默认为`(11,0,3,button_buf)`,即1、4、12按钮同时按下切换开关。
## 电路板使用说明
...
亚克力外壳分为大小两款，小款在`small`文件夹下，文件名带有3dprint的为使用3d打印盘托的外壳文件<br>
